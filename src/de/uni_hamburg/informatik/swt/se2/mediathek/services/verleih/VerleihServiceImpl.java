        ...
        implements VerleihService
{
    ...
    */
    ...
    @Override
    public void vormerkenAn(List<Medium> medien, Kunde kunde) {
    	...
    	for(Medium medium : medien)  {
    		...
    		if(vormerkkarte != null) {
    			vormerkkarte.fuegeKundehinzu(kunde);
    		} else {
    ...
    public boolean istVormerkenMoeglich(List<Medium> medien, Kunde kunde) {
    	...
    	for(Medium medium : medien)  {    		
    		...
    		if(vormerkkarte != null) {
    			if(vormerkkarte.istVormerkkarteVoll() || vormerkkarte.isKundeEnthalten(kunde)) {
        			return false;
        		}
    		}
    		if(verleihkarte != null && kunde.equals(verleihkarte.getEntleiher())) {
    			return false;
    		}
    	}
    	return true;
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            ...
            Vormerkkarte vormerkkarte = _vormerkkarten.get(medium);
            if(vormerkkarte != null) {
            	vormerkkarte.entferneErsteKundeVonListe();
            }
	...
	@Override
	public boolean istKundeInVormerkliste(List<Medium> medien, Kunde kunde) {
		assert kundeImBestand(
                kunde) : "Vorbedingung verletzt: kundeImBestand(kunde)";
        
		boolean istVorgemerkt = true;
        ...
        for(Medium medium: medien) {
        	...
        	Vormerkkarte vormerkkarte = _vormerkkarten.get(medium);
        	if (vormerkkarte != null) {
        		Kunde vormerkerErstePosition = vormerkkarte.getKundeAnPosition(0);
            	if (vormerkerErstePosition != null) {
            		if(!vormerkerErstePosition.equals(kunde)) {
            			istVorgemerkt = false;
            		}       		
            	}
        	}
        }
		return istVorgemerkt;
	}
